...
If a `Cell` or other reactive instance from `select/state/cells.js` is passed as a property or bound to a tracked attribute of the custom element, Select automatically subscribes to its updates and triggers a re-render of the component when the reactive value changes.
