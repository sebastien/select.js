...
function sortable(event, optionsOrDrop) {
	...
	const { preview, box, ox, oy } = createSortablePreview(
		sourceNode,
		event,
		options,
	);
	...
	return drag(
		...
		(ev) => {
			session.drop =
				locateSortablePlaceholder(session, ev.clientX, ev.clientY, ev) ??
				session.drop;
...
function sortableOptions(optionsOrDrop) {
	const options =
		typeof optionsOrDrop === "function"
			? { onDrop: optionsOrDrop }
			: (optionsOrDrop ?? {});
	return {
...
function sortableItemId(node, options) {
	return (
		node.getAttribute(attributeName(options.item, "data-sortable-item")) ??
		node.dataset.sortableItem
	);
}
...
function sortableIndex(node, listNode, options) {
	...
	const nodes = sortableListItems(listNode, {
		sourceNode: null,
		placeholder: null,
		options,
	});
...
function locateSortablePlaceholder(session, x, y, event) {
	...
	const custom = session.options.place?.(
		proposed,
		sortableState(session, event),
	);
	if (custom === false) {
	...
	if (!next) {
		...
		return {
			listNode: list,
			listId: sortableListId(list, session.options),
			index: 0,
			row: 0,
			col: 0,
		};
	...
	for (const node of listNode.children) {
		...
		if (node === session.placeholder) {
			return {
				...session.placement,
				listNode,
				listId: sortableListId(listNode, session.options),
				index,
			};
		}
		if (sortableMatches(node, session.options.item)) {
...
function updateSortablePreview(session, event) {
	...
	if (placeholder?.isConnected && placeholder.getBoundingClientRect) {
		...
		const dist = Math.sqrt(
			distanceSquared(event.clientX, event.clientY, cx, cy),
		);
		...
		height =
			session.previewHeight + (box.height - session.previewHeight) * ease;
	}
