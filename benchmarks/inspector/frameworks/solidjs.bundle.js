...
var IS_DEV = false;
var equalFn = (a, b) => a === b;
var _$PROXY = Symbol("solid-proxy");
var _$TRACK = Symbol("solid-track");
var _$DEVCOMP = Symbol("solid-dev-component");
var signalOptions = {
	equals: equalFn,
};
var ERROR = null;
...
function createSignal(value, options) {
	...
	const setter = (value2) => {
		if (typeof value2 === "function") {
			if (Transition?.running && Transition.sources.has(s))
...
function createRenderEffect(fn, value, _options) {
...
var [_transPending, setTransPending] = /* @__PURE__ */ createSignal(false);
var SuspenseContext;
function readSignal() {
...
function writeSignal(node, value, isComp) {
	const current =
		Transition?.running && Transition.sources.has(node)
			? node.tValue
			: node.value;
	if (!node.comparator?.(current, value)) {
		if (Transition) {
...
function createComputation(fn, init, pure, state = STALE, _options) {
	const c = {
		fn,
		state,
		updatedAt: null,
	...
	if (Owner === null);
		...
		if (Transition?.running && Owner.pure) {
...
function cleanNode(node) {
	...
	if (Transition?.running && node.pure) {
		reset(node, true);
	} else if (node.owned) {
	...
	if (Transition?.running) node.tState = 0;
	else node.state = 0;
}
...
function handleError(err, owner = Owner) {
	const fns = ERROR && owner?.context?.[ERROR];
...
function eventHandler(e) {
	if (sharedConfig.registry && sharedConfig.events) {
		if (sharedConfig.events.find(([_el, ev]) => ev === e)) return;
	}
	let node = e.target;
	const key = `$$${e.type}`;
