...
import { access, eq, isObject, Nothing, path as pathify, reassign, sanitizeValue } from "./utils.js"
...
class RecordFormat {
	...
	static sanitizeText(value, warn, scope, details = {}) {
		...
		for (let i = 0; i < text.length; i++) {
			...
			if ((code >= 0x00 && code <= 0x1f) || code === 0x7f || code === 0x2028 || code === 0x2029) continue
			sanitized += text[i]
		}
		if (sanitized !== text) {
	...
	static sanitizeKey(key, warn, scope, details = {}) {
		...
		if (
			...
			RecordFormat.warnIssue(warn, scope, "unsafe key pruned", { key, sanitized, ...details })
			return undefined
		}
		return sanitized
	}
	...
	static sanitizeItem(value, warn, scope, details = {}) {
		...
		if (typeof value === "string") return RecordFormat.sanitizeText(value, warn, scope, details)
		if (typeof value === "number") {
			if (Number.isFinite(value)) return value
			RecordFormat.warnIssue(warn, scope, "non-finite number pruned", { value, ...details })
			return undefined
		}
	...
	static sanitizeArray(value, warn, scope, details = {}) {
		...
		for (let i = 0; i < value.length; i++) {
			const item = RecordFormat.sanitizeItem(value[i], warn, scope, { ...details, index: i })
	...
	static sanitizeRecord(value, warn, scope) {
		if (!isObject(value)) {
			if (value !== undefined && value !== null) {
				RecordFormat.warnIssue(warn, scope, "unsupported location container pruned", {
					type: typeof value,
					value,
				})
			}
		...
		for (const key in value) {
			...
			if (Array.isArray(item)) {
				const safeArray = RecordFormat.sanitizeArray(item, warn, scope, { key: safeKey })
				if (safeArray.length) res[safeKey] = safeArray
			} else {
				const safeValue = RecordFormat.sanitizeItem(item, warn, scope, { key: safeKey })
...
class PathFormat {
	...
	parse(value) {
		...
		for (let i = 0; i < segments.length; i++) {
			try {
				...
				RecordFormat.warnIssue(this.warn, "browser.path", "path segment decode failed", {
					error,
					segment: segments[i],
					index: i,
				})
	...
	format(value) {
		const text = PathFormat.sanitizeText(value, this.warn)
		const segments = text.split("/")
		for (let i = 0; i < segments.length; i++) segments[i] = encodeURIComponent(segments[i])
		return segments.join("/") || "/"
	}

	read(win, fallback = "/") {
		return win?.location ? this.parse(win.location.pathname) : PathFormat.sanitizeText(fallback, this.warn)
	}
}
...
class HashFormat extends RecordFormat {
	...
	static formatAtom(value) {
		...
		const text = `${value}`
		return HashFormat.RE_HASH_ESCAPE.test(text) ? `"${text.replaceAll('"', '\\"')}"` : text
	}
	...
	static parseHash(value) {
		const source = RecordFormat.sanitizeText(value, undefined, "browser.hashformat")
		const root = []
		const stack = [root]
		...
		const commit = (atom) => {
			...
			if (key !== undefined) {
				if (Array.isArray(container)) {
					...
					const replacement = {}
					for (let i = 0; i < container.length; i++) replacement[i] = container[i]
		...
		while (cursor < source.length) {
			if (rawStart >= 0) {
				...
				if (quote < 0) {
					const raw = HashFormat.decodeComponent(source.substring(rawStart).replaceAll('\\"', '"'))
					if (raw !== "") commit(raw)
					break
				}
				if (source[quote - 1] === "\\") {
	...
	decodeText(value) {
		return RecordFormat.sanitizeText(`${value || ""}`.replace(/^#/, ""), this.warn, this.scope)
	}
...
class QueryFormat extends RecordFormat {
	...
	decodeText(value) {
		...
		if (i >= 0) {
			...
			RecordFormat.warnIssue(this.warn, this.scope, "query hash fragment pruned", {
				value: normalized,
				sanitized: pruned,
			})
			return pruned
...
const QuerySerializer = {
	...
	format(value) {
		return HashFormat.formatHash(RecordFormat.sanitizeRecord(value, undefined, "browser.query"))
	},
}
...
class LocationState {
	constructor(options = {}) {
		...
		this.warn =
			typeof options.warn === "function"
				? options.warn
				: (scope, error, details = {}) =>
						logSelectBrowser("warn", scope, error?.message || "browser warning", {
							error,
							...details,
						})
		this.mode = options.mode === "push" ? "push" : "replace"
		...
		this.path = new LocationValueCell(this.pathFormat.read(this.hasWindow ? this.win : undefined, options.path || "/"), {
			mode: this.mode,
			normalize: (value) => this.pathFormat.parse(value),
			writer: (_value, settings) => this.writeURL(settings.mode),
		})
		this.query = new LocationValueCell(this.queryFormat.read(this.hasWindow ? this.win : undefined, {}), {
			mode: this.mode,
			merge: true,
			normalize: (value) => this.queryFormat.sanitizeRecord(value),
			writer: (_value, settings) => this.writeURL(settings.mode),
		})
		this.hash = new LocationValueCell(this.hashFormat.read(this.hasWindow ? this.win : undefined, {}), {
			mode: this.mode,
			merge: true,
	...
	static getWindow() {
		return typeof globalThis !== "undefined" && globalThis.window ? globalThis.window : undefined
	}
	...
	writeURL(mode = this.mode) {
		...
		const url = this.formatURL(this.path.value, this.query.value, this.hash.value)
	...
	syncFromLocation() {
		const nextPath = this.pathFormat.read(this.hasWindow ? this.win : undefined, this.path.value)
		const nextQuery = this.queryFormat.read(this.hasWindow ? this.win : undefined, this.query.value || {})
		const nextHash = this.hashFormat.read(this.hasWindow ? this.win : undefined, this.hash.value || {})
	...
	bind() {
		if (!this.hasWindow || typeof this.win.addEventListener !== "function") return
...
function browser(options = {}) {
	...
	const local = (key, dflt, opts = {}) => {
		...
		const serializer =
			opts && typeof opts.parse === "function" && typeof opts.format === "function"
				? opts
				: localSerializer
		const initial = hasStorage
			? (() => {
					const raw = win.localStorage.getItem(key)
					return raw === null ? dflt : location.safeParse(`browser.local:${key}`, serializer, raw, dflt)
				})()
			: dflt
	...
	return { path: location.path, query: location.query, hash: location.hash, local }
}
