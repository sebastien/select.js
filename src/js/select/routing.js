...
function route(text) {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (item === "*") {
			...
			res.push(
				new RoutePatternSlot(
					pattern(wildcardPattern(item)),
					item,
					res.length,
					false,
				),
...
class Router {
	...
	off(expr, handler = undefined, offset = 0) {
		...
		if (offset === rte.length) {
			this.handlers = handler
				? this.handlers.filter((h) => h.value !== handler)
				: [];
		} else if (chunk instanceof RouteDescendant) {
			this.descendants = handler
				? this.descendants.filter((h) => h.value !== handler)
				: [];
		} else if (typeof chunk === "string") {
	...
	match(p, offset = 0) {
		...
		if (this.static.has(chunk)) {
			const staticMatches =
				this.static.get(chunk)?.match(items, offset + 1) ?? null;
...
function routed(routes = undefined) {
	...
	return Object.assign((p, ...args) => r.run(p, ...args), {
		router: r,
		match: r.match.bind(r),
	});
}
