...
@function notIdentical value, expected
	if value is expected
		return fail ("not identical: '" + format (value) + "' === '" + format (expected) + "'")
	else
		return succeed ()
	end
@end
...
@class HTMLReporter
	...
	| results, otherwise creates it.
		if (not ui) or (ui length == 0) or (not uis table) or (uis table length == 0)
