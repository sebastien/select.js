...
function floor(value, factor = undefined) {
	return factor === undefined ? unary(value, Math.floor) : round(value, factor, -1)
}

// Function: ceil
// Returns the ceil of `value`, preserving list shape when possible.
function ceil(value, factor = undefined) {
	return factor === undefined ? unary(value, Math.ceil) : round(value, factor, 1)
}
...
function scale(value, fromRange, toRange = undefined) {
	...
	const offset = (value - fromRange[0]) / (fromRange[fromRange.length - 1] - fromRange[0] || 1)
...
function step(start, end, count = 10, base = 10) {
	...
	return sign(end - start) * round(base ** scaleOrder / count, base ** stepOrder)
}
