...
function createDefinitionComponent(name = null, scope = document) {
	...
	const component = (..._args) => {
		throw new Error(`ui.component(${JSON.stringify(name)}): definition is not bound to a template; call .using(...) before rendering`)
	}
	Object.assign(component, {
		...
		singleton: null,
		new: (..._args) => {
			throw new Error(`ui.component(${JSON.stringify(name)}): definition is not bound to a template; call .using(...) before .new(...)`)
		},
		apply: (..._args) => {
			throw new Error(`ui.component(${JSON.stringify(name)}): definition is not bound to a template; call .using(...) before .apply(...)`)
		},
		map: (..._args) => {
			throw new Error(`ui.component(${JSON.stringify(name)}): definition is not bound to a template; call .using(...) before .map(...)`)
		},
		using: (selection, boundScope = scope) => createTemplateComponent(selection, boundScope, definition),
		init: (init) => {
...
function normalizeResourceURL(url, scope = document) {
	const baseURL = scope?.baseURI || document.baseURI || globalThis.location?.href
...
function cloneTemplateSourceNodes(templateNode) {
	return [...templateNode.content.childNodes].map((node) => node.cloneNode(true))
}
...
function getTemplateResource(ref, scope = document) {
	const parsed = typeof ref === "string" ? parseResourceReference(ref, scope) : ref
	if (!parsed) {
		return null
	}
	const resource = TEMPLATE_RESOURCES.get(parsed.url)
	if (!resource) {
		throw new Error(`ui(): template resource not loaded: ${parsed.url}; call ui.load(${JSON.stringify(parsed.url)}) first`)
	}
	if (!parsed.key) {
	...
	if (!templateNode) {
		throw new Error(`ui(): template ${JSON.stringify(parsed.key)} not found in ${parsed.url}`)
	}
...
function createTemplateComponent(selection, scope = document, definition = null) {
	const appliedDefinition = definition ? cloneDefinitionState(definition) : null
	if (typeof selection === "string") {
		...
		if (resourceRef) {
			...
			if (!resolved?.templateNode) {
				throw new Error(`ui(): external template reference requires a #Template name: ${selection}`)
			}
			...
			if (/^\s*</.test(selection)) {
				...
				if (template) {
					...
					if (recent?.templateNode) {
						...
						autoFormatName = TemplateRegistry.FormatterName(recent.templateNode)
					}
				}
				if (nodes.length === 0) {
						...
						for (const node of matchedNodes) {
							...
							if (payload !== null && payload !== undefined && payload !== "") {
								...
								try {
									...
									throw new Error(`ui(): invalid JSON in [data] attribute for selector ${JSON.stringify(selection)}`)
								}
								if (parsedPayload === null || typeof parsedPayload !== "object" || Array.isArray(parsedPayload)) {
									throw new Error(`ui(): [data] attribute JSON for selector ${JSON.stringify(selection)} must be an object`)
								}
...
function createComponent(tmpl, localTemplateNodes = tmpl.nodes, definition = null) {
	...
	Object.assign(component, {
		...
		using: (selection, scope = tmpl.scope) => createTemplateComponent(selection, scope, localDefinition),
		init: (...args) => {
